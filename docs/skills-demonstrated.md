...
- **eBPF & Network Policies** — Cilium CNI (v1.20.1) kube-proxy replacement, Gateway API, and fine-grained `CiliumNetworkPolicy` (`cilium.io/v2`) across all platform namespaces with L7 DNS filtering and unblocked dynamic storage data planes. See `platform/*/templates/cilium-networkpolicies.yaml` and [ADR-014](adrs/014-cilium-cni-and-identity-networkpolicies.md).
