...
| `secret "cloud-credentials" not found` / `velero-bucket-init` fails | `init-sops.sh` not run (SOPS secret missing) and no `AWS_*` env for the `ensureVeleroCredentials()` fallback | `just secrets-apply`, or `AWS_ACCESS_KEY_ID=... AWS_SECRET_ACCESS_KEY=... ./bootstrap/init-gitops.sh prod`; `kubectl -n velero logs job/velero-bucket-init` |
