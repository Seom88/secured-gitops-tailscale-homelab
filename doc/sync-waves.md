...
| 5 | `-2` | `argocd-sync` | ArgoCD manages its own versioning via `platform/argocd/` chart |
| 6 | `-2` | `eso-sync` | Installs External Secrets Operator + creates `ClusterSecretStore` and `ExternalSecret` |
| 7 | `-1` | `tailscale-operator` | Deploys Tailscale operator using credentials from Vault via ESO |
...
5. **ArgoCD** (`-2`): Self-managed via its own Application manifest, allowing version control through Git commits
6. **ESO** (`-2`): External Secrets Operator reads from Vault using `ClusterSecretStore` (path: `secret/`)
7. **Tailscale** (`-1`): Uses the `operator-oauth` secret created by ESO
